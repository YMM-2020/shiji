...
// ── SQLite wrapper (sql.js → better-sqlite3-like API) ──
...
async function initDb(): Promise<DbWrapper> {
  ...
  if (fs.existsSync(DB_PATH)) {
    ...
    console.log('SQLite database created in memory, will save to', DB_PATH)
    // Auto-save new DB
...
function saveDb(wrapper: DbWrapper) {
  // sql.js doesn't expose internal handle easily; we'll save on shutdown
}
...
async function start() {
  ...
  app.post('/api/auth/send-code', (req, res) => {
    ...
    const code = '1234'
    db.prepare('INSERT INTO verification_codes (phone, code, created_at) VALUES (?, ?, ?)').run(phone, code, new Date().toISOString())
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`  GET /api/export/markdown`)
    console.log(`  GET /api/export/prompt`)
    console.log(`  GET /api/export/compact`)
  })
}
